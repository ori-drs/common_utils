...
void quaternionToBotDouble(double quat[4], const Eigen::Quaterniond & eig_quat)
{
  quat[0] = eig_quat.coeffs()(3);
  quat[1] = eig_quat.coeffs()(0);
  quat[2] = eig_quat.coeffs()(1);
  quat[3] = eig_quat.coeffs()(2);
}

void botDoubleToQuaternion(Eigen::Quaterniond & eig_quat, const double quat[4])
{
  eig_quat.coeffs()(3) = quat[0];
  eig_quat.coeffs()(0) = quat[1];
  eig_quat.coeffs()(1) = quat[2];
  eig_quat.coeffs()(2) = quat[3];
}
...
void convertAndUnwrapYaw(const Eigen::Quaterniond& q, const double& yaw_prev, Eigen::Vector3d& rpy){
  rpy = getEulerAngles(q);
  rpy(2) = unwrap(yaw_prev, rpy(2));
}
} // namespace eigen_utils
